...
class Ui_checkin_window(QWidget):
    ...
    def setupUi(self, checkin_window, room_num):
        ...
        try:
            ...
            if data:
                self.lineEdit_name = MyLineEdit(checkin_window)
                self.lineEdit_phone = MyLineEdit(checkin_window)

                self.lineEdit_name.setText(data[1])
                self.lineEdit_phone.setText(data[2])
            else:
    ...
    def retranslateUi(self, checkin_window):
        ...
        self.label_day_leave.setText(_translate("checkin_window", "退房日期"))
    ...
    def checkin(self):
        ...
        cur_year = str(cur_date[0])
        cur_month = str(cur_date[1])
        cur_day = str(cur_date[2])
        # print(cur_year,cur_month,cur_day)
        today = cur_year + '-' + cur_month + '-' + cur_day

        new_date = self.lineEdit_day_leave.text()
        new_year = new_date[:4]
        ...
        if int(cur_year)>int(new_year):
            QMessageBox.warning(self, '警告', '退房日期不能早于\n入住日期:%s'%today, QMessageBox.Ok)
            return
        ...
        #     return
        # elif data[5] == '入住':
        #     QMessageBox.information(self, '登记失败', '此房间已经有客', QMessageBox.Ok)
        #     return
            
        try:
            sql_delete = "delete from book_info where room_num = '%s'"%self.room_num
